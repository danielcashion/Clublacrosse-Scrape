...
class TmachineextractorSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        try:
            ...
            self.cursor.execute(
                "SELECT [link] FROM [data].[tm_mainpage_data] WHERE 1=1 AND end_date < GETDATE() AND is_active = 1 AND 'https://tourneymachine.com/' + link NOT IN (SELECT tournament_endpoint FROM [clubstats].[stg].[tourneymachine_data]) ORDER BY end_date")
